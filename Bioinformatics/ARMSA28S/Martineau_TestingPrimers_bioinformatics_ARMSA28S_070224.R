...
load_blast(blast_tab_file = "blast_results_ARMS17.tab", db_name = "ARMS17")

## C.2 Filter nt blast results
filter_blast (raw_blastn = ARMS17_blastn_file_raw,
              min_perc_id = 95,
              min_query_cov = 95,
              sequences_abundance_path = "Martineau_TestingPrimers_Clean_ARM17_abundance_sequences_071224.rarefied.csv",
              abundance_path = "Martineau_TestingPrimers_Clean_ARM17_abundance_071224.rarefied.csv",
              db_name = "ARMS17",
...
#######################################################################
########  D. Delete duplicates (manual filtration for ARMS 17) ########
#######################################################################
...
non_dupes <- ARMS17_filtered_blast %>%
  filter(!(query.id %in% unique(dupes$query.id)))

# Filter duplicated ASVs
dupes_filtration <- ARMS17_filtered_blast %>%
  filter(query.id %in% unique(dupes$query.id)) %>%
  select(ID_ASV = query.id, species = scientific.name, subject = subject.id)

# Merge abundance table and species
dupes_filtration_merged <- right_join(ARMS17_abundance_table %>%
                                        mutate(ID_ASV = V1) %>%
                                        select(-V1), dupes_filtration)

# Convert to long format
dupes_filtration_long <- dupes_filtration_merged %>%
  pivot_longer(cols = ARMS17R1:"%cover17R3",
               names_to = "Sample",
               values_to = "values") %>%
  mutate(values = as.numeric(values))

# Duplicates Eq17
# Find duplicates that are present in ARMS17
species.in.cocktail <-  c("Haliclona sp. 1 JV-2020",          
...
duplicates_eq17 <- dupes_filtration_long %>%
  filter(Sample %in% c("Equimolar17R1", "Equimolar17R2", "Equimolar17R3") & values > 0 & species %in% species.in.cocktail)

# PC 17
# Select the duplicates that are left (not in eq20, not in eq18) and remove them from the long data set
dupes_filtration_long <- dupes_filtration_long[which(!(dupes_filtration_long$ID_ASV %in% duplicates_eq17$ID_ASV)), ]

# Among duplicates left, find the ones that are in cocktail pc20
duplicates_pc17 <- dupes_filtration_long %>%
  filter(Sample %in% c("%cover17R1", "%cover17R2", "%cover17R3") & values > 0 & species %in% species.in.cocktail)

# ARMS 17
# Select the duplicates that are left (not in eq20, not in eq18, not in PC20, not PC18) and remove them from the long data set
dupes_filtration_long <- dupes_filtration_long[which(!(dupes_filtration_long$ID_ASV %in% duplicates_pc17$ID_ASV)), ]

# Among duplicates left, find the ones that are in cocktail ARMS20
duplicates_ARMS17 <- dupes_filtration_long %>%
  filter(Sample %in% c("ARMS17R1", "ARMS17R2", "ARMS17R3") & values > 0 & species %in% species.in.cocktail)

# Inspect what is left: not in eq, not in pc and not in ARMS
dupes_filtration_long <- dupes_filtration_long[which(!(dupes_filtration_long$ID_ASV %in% duplicates_ARMS17$ID_ASV)), ]
...
# Select the duplicates identified in the cocktails: only for Eq17, because there are no duplicates in PC or ARMS 17
# Label the inspected ones as "inspected"
clean_duplicated_blast_search <- dupes_filtration_long_clean %>%
  dplyr::mutate(species_new = case_when ((ID_ASV %in% duplicates_eq17$ID_ASV)& (Sample  %in% c("Equimolar17R1","Equimolar17R2", "Equimolar17R3")) & (species %in% species.in.cocktail) ~ species,
  ...
  dplyr::filter(!(duplicated(ID_ASV, species)))   #Add this last line because there are duplicates lines from R1, R2, R3
...
duplicates_keep_df <- left_join (duplicates_keep, ARMS17_filtered_blast)
...
length(unique(ARMS17_filtered_blast$query.id)) == length(unique(no.duplicated.blast.searches_1$query.id))
...
classify_blast(blastn_result = no.duplicated.blast.searches_1, taxonomy_vector = c("kingdom", "phylum", "class", "order", "family", "genus", "species"), db_name = "ARMS17")



#############################################
########  F. Taxonomic Replacements  ########
#############################################

## E.1 Data wrangling 
# Add taxonomic information to the abundance table
ASV_fp <- ARMS17_abundance_table %>%
  dplyr::rename(ID = V1)
rownames(ASV_fp) <- ASV_fp$ID

ASV_fp_long <- right_join(ASV_fp, ARMS17_taxonomy_table)%>%
...
species_cocktail <- function (input_sample, species_list) {
  cocktails_df <- ASV_fp_long%>%
    dplyr::filter(Sample %in% input_sample) %>%
    dplyr::filter(values > 0) %>%
...
cocktails_df <- species_cocktail(input_sample = c("Equimolar17R1", "Equimolar17R3", "Equimolar17R2"), species_list = species.in.cocktail)
...
cocktails <- list(
  "Equimolar" = c("Equimolar17R1","Equimolar17R3","Equimolar17R2"),
  "%cover" = c("%cover17R1", "%cover17R2","%cover17R3"),
  "ARMS" = c("ARMS17R1", "ARMS17R2", "ARMS17R3"))

# Loop over cocktails
for (i in seq_along(cocktails)) {
  sample_name <- names(cocktails[i])
  vector_name <- c(cocktails[[i]])
  # Name each data frame based on the cocktail names 
  species_cocktails_list[[paste("species_cocktail", sample_name, sep = "_")]] <- 
    species_cocktail(input_sample = vector_name, species_list = species.in.cocktail)
}

# Check if Equimolar 20 df is the same as expected
nrow(species_cocktails_list[["species_cocktail_Equimolar"]]) == nrow(cocktails_df)
...
swaps_eq17 <- list(
  ...
  list(from = "Haliclona sp. 23 JV-2020", to = "Haliclona sp. 2 JV-2020"),
  list(from = "Leucosolenida sp. 10 JV-2020", to = "Leucosolenida sp. 7 JV-2020")
)

swaps_ARMS17 <- list(
  list(from = "Tethyidae sp. 1 JV-2020", to = "Tethya sp. 5 JV-2020"),
  list(from = "Suberitidae sp. 2 JV-2020", to = "Suberitidae sp. 1 JV-2020"),
  list(from = "Clathrinidae sp. 11 JV-2020", to = "Clathrinidae sp. 4 JV-2020"),
  list(from = "Tethya sp. 6 JV-2020", to = "Tethya sp. 5 JV-2020"),
  list(from = "Suberitidae sp. 4 JV-2020", to = "Suberitidae sp. 1 JV-2020"),
  list(from = "Clathrinidae sp. 3 JV-2020", to = "Clathrinidae sp. 4 JV-2020"),
  list(from = "Clathrinidae sp. 8 JV-2020", to = "Clathrinidae sp. 4 JV-2020"),
  list(from = "Clathrinida sp. 2 JV-2020", to = "Clathrinida sp. 1 JV-2020"),
  list(from = "Haliclona laubenfelsi", to = "Haliclona sp. 1 JV-2020")
)

# Apply manual swaps for each cocktail
ASV_fp_long_modified <- manual_swaps(ASV_fp_long, "Equimolar", swaps_eq17)
ASV_fp_long_modified <- manual_swaps(ASV_fp_long_modified, "ARMS", swaps_ARMS17)

## E.4 Verifications
# Make same function but for verification
...
cocktails_df_verify  <- species_cocktail_verify(input_sample =  c("Equimolar17R1", "Equimolar17R3", "Equimolar17R2"), species_list = species.in.cocktail)
...
for (i in seq_along(cocktails)) {
  ...
  species_cocktails_list_verify[[paste("verify_species_cocktail", sample_name, sep = "_")]] <- 
    species_cocktail_verify(input = vector_name, species_list =  species.in.cocktail)
}

# Check if Equimolar 20 df is the same as expected
nrow(species_cocktails_list_verify[["verify_species_cocktail_Equimolar"]]) == nrow(cocktails_df_verify)
...
new_abundance <- ASV_fp_long_modified %>%
  ...
  pivot_wider(values_from = values,
              names_from = Sample) %>%
  as.data.frame()
nrow(new_abundance)== nrow(ARMS17_taxonomy_table)

# Extract taxonomy from all species
taxo_join <- ARMS17_taxonomy_table %>%
...
# Save that df in the working directory
write.csv(new_taxo, "./outputs/ARMS17_taxonomy_table_swaps.csv", row.names = TRUE)
#Final cleaned table
write.csv(new_abundance,"./outputs/ARMS17_abundance_table_swaps.csv", row.names = TRUE)
...
load_blast(blast_tab_file = "blast_results_ARMS17_nt.tab", db_name = "ntARMS17")

## F.2 Filter nt blast results
filter_blast (raw_blastn = ntARMS17_blastn_file_raw,
              min_perc_id = 95,
              min_query_cov = 95,
              sequences_abundance_path = "Martineau_TestingPrimers_Clean_ARM17_abundance_sequences_071224.rarefied.csv",
              abundance_path = "Martineau_TestingPrimers_Clean_ARM17_abundance_071224.rarefied.csv",
...
delete_duplicated_ID(filtered_blastn = ntARMS17_filtered_blast,
                     db_name = "ntARMS17")

## F.4 Classify the results
classify_blast(blastn_result = ntARMS17_no_duplicates, taxonomy_vector = c("kingdom", "phylum", "class", "order", "family", "genus", "species"), db_name = "ntARMS17")
...
merge_taxo_abundance(taxonomyA = "ARMS17_taxonomy_table_swaps.csv",
                     taxonomyB = "ntARMS17_taxonomy_table.csv",
                     abundance_global =  "Martineau_TestingPrimers_Clean_ARM17_abundance_071224.rarefied.csv",
                     filename = "sponge_ntARMS17")
